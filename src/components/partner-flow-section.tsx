...
const FALLBACK_STEPS = [
  ...
  },
  {
    ...
    description:
      'Jenis makanan, jumlah makanan, jadwal penjemputan donasi dan lokasi donatur',
    imagePlaceholder: 'heart-smile',
  },
  {
    title: 'Tim Garda Pangan akan menjemput donasi',
    description:
      'Donasi akan disalurkan kepada masyarakat pra sejahtera di Surabaya',
    imagePlaceholder: 'heart-smile',
  },
  {
    title: 'Laporan bulanan dari tim Garda Pangan',
    description:
      'Laporan bulanan akan dikirim mengenai lokasi distribusi donasi, sebagai pertanggungjawaban.',
...
export function PartnerFlowSection({ data }: { data?: MitraData }) {
  ...
  const flowDescription =
    ...
    'Garda Pangan menjalin kerjasama dengan Mitra (mitra dari restoran, hotel, bakery, cafe, rumah makan, katering, mall, distributor, FMCG, dan berbagai bisnis makanan lainnya) yang berkomitmen mendonasikan makanan surplus layak makan yang dihasilkan. Food rescue dilakukan setiap harinya dengan menjemput makanan yang tidak terjual dari mitra, untuk didistribusikan langsung secara gratis ke panti asuhan/masyarakat yang membutuhkan.'
    ...
    <SectionShell tone="white">
      ...
      <div className="relative mx-auto max-w-4xl pt-8 font-serif">
        ...
        <div className="space-y-12 md:space-y-8">
          {(data?.flowSteps || FALLBACK_STEPS).map(
            (step: any, index: number) => {
              const isEven = index % 2 === 0
              const number = index + 1
              const buttons = []
              if (step.button1Label)
                buttons.push({
                  label: step.button1Label,
                  link: step.button1Link || '#',
                })
              else if (step.buttons?.[0])
                buttons.push({ label: step.buttons[0], link: '#' })
              if (step.button2Label)
                buttons.push({
                  label: step.button2Label,
                  link: step.button2Link || '#',
                })
              else if (step.buttons?.[1])
                buttons.push({ label: step.buttons[1], link: '#' })
                      ...
                      <div className="relative z-10">
                        <div className="mb-6 w-full flex items-center justify-center">
                          {step.icon?.url ? (
                            ...
                            <div className="h-40 w-40 bg-garda-paper/50 flex items-center justify-center rounded-xl text-garda-ink-soft/40 uppercase tracking-widest text-xs font-bold border border-garda-forest/10">
                              {step.imagePlaceholder} img
                            </div>
                          )}
                        </div>
                          ...
                          <div className="mt-8 flex flex-col items-center gap-3 max-w-[280px] mx-auto">
                            {buttons.map((btn, i) => (
                              <a
                                key={i}
                                href={btn.link}
                                className="w-full flex justify-between items-center bg-garda-forest text-white rounded-full pl-6 pr-2 py-2 text-xs font-semibold hover:bg-garda-forest/90 transition-colors shadow-sm"
                              >
